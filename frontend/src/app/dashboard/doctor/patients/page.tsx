...
import { Search, Eye, Activity, Calendar, FileText, ArrowLeft } from 'lucide-react';
        ...
        {selectedPatientId && patientDetails ? (
          <div className="space-y-6">
            <button
              ...
              className="flex items-center gap-2 text-slate-400 hover:text-slate-200 transition-colors text-sm font-medium animate-pulse"
            >
                  ...
                  <form onSubmit={handleVitalsSubmit} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="37.2"
                        className="w-full px-4 py-2 bg-slate-900 border border-slate-855 rounded-xl text-sm"
                        required
                      />
                    </div>
                    <div>
                  ...
                  <div className="space-y-4">
                    {patientDetails.vitals?.length === 0 ? (
                      ...
                      patientDetails.vitals.slice(0, 5).map((vit: any) => (
                        <div key={vit.id} className="p-4 bg-slate-900/60 border border-slate-855 rounded-2xl text-xs space-y-2">
                      ...
                      {patientDetails.appointments.map((app: any) => (
                        <div key={app.id} className="py-3 flex justify-between items-center text-xs">
                          ...
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                            ...
                            app.status === 'PENDING' ? 'bg-amber-950 text-amber-450' : 'bg-slate-900 text-slate-400'
                          }`}>
                            {app.status}
                          </span>
                        </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <h2 className="text-2xl font-bold tracking-tight text-slate-100">Patient Electronic Health Records</h2>
                <p className="text-slate-400 text-sm">Access patient profiles and vital signs histories.</p>
              </div>
              ...
              <form onSubmit={handleSearch} className="flex gap-2 w-full max-w-sm">
                <input
                  ...
                  className="w-full px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-sm focus:border-teal-500 focus:outline-none"
                />
                <button
                  type="submit"
                  ...
                  return (
                    <div key={pat.id} className="p-6 bg-slate-950 border border-slate-850 rounded-3xl shadow-md flex flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between border-t border-slate-850 pt-4 mt-4">
                        <span className="text-[9px] text-slate-550 font-mono">DOB: {new Date(pat.dateOfBirth).toLocaleDateString()}</span>
                        <button
