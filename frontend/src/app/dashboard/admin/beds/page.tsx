...
import { BedDouble, Check, AlertTriangle, UserPlus, LogOut, Wrench } from 'lucide-react';
...
export default function BedsManagement() {
  ...
  const { emit } = useSocket(undefined, (event, data) => {
    if (event === 'bed_updated') {
      console.log('Real-time Bed event payload received:', data);
      setBeds((prevBeds) =>
        prevBeds.map((bed) =>
          bed.id === data.bedId
            ? { 
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-100">Ward Beds Occupancy</h2>
                  ...
                  <div>
                    ...
                    <div className="flex justify-between items-start border-b border-slate-850/60 pb-3 mb-4">
                      <div className="flex items-center gap-3">
                        <BedDouble className={`h-6 w-6 ${
                          ...
                          isMaintenance ? 'text-rose-400' : 'text-emerald-400'
                        }`} />
                        <div>
                    ...
                    {isOccupied && activeAssignment ? (
                      <div className="space-y-2 text-xs">
                        ...
                        <p className="text-[9px] text-slate-500">Admitted: {new Date(activeAssignment.assignedAt).toLocaleString()}</p>
                      </div>
                    ) : (
                      <p className="text-xs text-slate-500 italic py-4">No patient currently admitted.</p>
                    )}
                  </div>

                  {/* Bed Action buttons */}
                  <div className="flex gap-2 mt-6 border-t border-slate-850/60 pt-4">
                    {isOccupied ? (
                      <button
                    ...
                    <button
                      ...
                      disabled={isOccupied}
                      className="px-3 py-2 rounded-xl bg-slate-900 hover:bg-slate-850 text-slate-400 hover:text-slate-200 border border-slate-800 text-xs transition-colors cursor-pointer"
                      title="Maintenance"
                    >
        ...
        {isAssignModalOpen && selectedBed && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm" onClick={() => setIsAssignModalOpen(false)} />
